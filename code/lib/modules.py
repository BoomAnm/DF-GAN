...
##sle_sdc
def train_sle_sdc(dataloader, netG, netD, netC, text_encoder, optimizerG, optimizerD, args):
    batch_size = args.batch_size
    device = args.device
    epoch = args.current_epoch
    max_epoch = args.max_epoch
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        # whole D loss
        errD = errD_real + (errD_fake + errD_mis) / 2.0 + errD_MAGP + \
               percept(rec_all, F.interpolate(imgs, rec_all.shape[2])).sum() + \
               percept(rec_small, F.interpolate(imgs, rec_small.shape[2])).sum() + \
               percept(rec_part, F.interpolate(crop_image_by_part(imgs, part), rec_part.shape[2])).sum()
...
def test(dataloader, text_encoder, netG, device, m1, s1, epoch, max_epoch,
         times=1, z_dim=100, batch_size=64, truncation=True, trunc_rate=0.86):
